...
StatusController.prototype.getInfo = function(callback) {
  ...
  var getCirculating = function(blockHeight) {
      var halvings         = 64;
      var halvingInterval  = 210000;
      var halvingsPassed   = Math.floor(blockHeight/halvingInterval);
      var genesisReward    = 50;
      var circulating      = 0;
      var currentBlock     = 0;
      var forkBlock        = 555555;
      var currentReward    = genesisReward;
      for (var i=1; i <= 64; i++) {
          if (i == 3 && blockHeight > forkBlock) {
              circulating += (forkBlock-currentBlock) * currentReward
              circulating += 6330450; // Allocations
              currentReward = currentReward*2;
              if (halvingsPassed < 3) {
                  circulating += ((blockHeight-forkBlock) * currentReward) - currentReward
                 	return circulating;
              }
              else {
                circulating += ((halvingInterval*i)-forkBlock) * currentReward
              }
              currentBlock += halvingInterval;

          }
          else if (blockHeight < halvingInterval*i) {
          	circulating += (blockHeight-currentBlock) * currentReward;
            return circulating;
          }
          else {
              currentBlock += halvingInterval;
              circulating += currentReward * halvingInterval;
          }
          currentReward = currentReward/2;
      }
      return circulating;
  ...
  this.node.services.bitcoind.getInfo(function(err, result) {
    ...
    var info = {
      ...
      circulating: getCirculating(result.blocks)
    };
    callback(null, info);
  });
};
